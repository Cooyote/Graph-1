...
namespace Graph
{
    ...
    AdjacencyMatrix::dijkstra_data AdjacencyMatrix::dijkstra(int begin, int end)
    {
        dijkstra_data data[this->num_vertices];

        bool visited[this->num_vertices];
        memset(visited,0, this->num_vertices * sizeof(bool));

        visited[begin] = 1;
        data[begin].path = std::to_string(begin);
        data[begin].distance = 0;

        for(int i = 0; i< num_vertices; i++)
        {
            if(this->matrix[begin * num_vertices + i].edge == 1)
            {
                data[i].distance = this->matrix[begin * num_vertices + i].value;
                data[i].path = std::to_string(begin) + " "+ std::to_string(i);
            }
        }

        int current_vertice = end;

        for(int i = 0; i < this->num_vertices; i++)
        {
            if(!visited[i] && data[i].distance < data[current_vertice].distance)
            {
                current_vertice = i;
            }
        }

        while(current_vertice != end)
        {
            visited[current_vertice] = 1;

            for(int i = 0; i< num_vertices; i++)
            {
                if(!visited[i] && this->matrix[current_vertice * num_vertices + i].edge == 1)
                {
                    if(data[i].distance > data[current_vertice].distance + this->matrix[current_vertice * num_vertices + i].value )
                    {
                        data[i].distance = data[current_vertice].distance + this->matrix[current_vertice * num_vertices + i].value;
                        data[i].path = data[current_vertice].path + " "+ std::to_string(i);
                    }
                }
            }
    ...
    void AdjacencyMatrix::bellman_ford_pass(int* data)
    {
        for(int i = 0; i< this->num_vertices; i++)
        {
            if(data[i] < 9999)
            {
                for(int j = 0; j < this->num_vertices; j++)
                {
                    if(this->matrix[i * num_vertices + j].edge == 1)
                    {
                        if(this->matrix[i * this->num_vertices + j].value + data[i] < data[j])
                        {
                            data[j] = this->matrix[i * this->num_vertices + j].value + data[i];
                        } 
                    }
                }
            }
    ...
    int AdjacencyMatrix::bellman_ford(int begin, int end)
    {
        int dists[this->num_vertices]; 

        int cmp[this->num_vertices]; 
        ...
        for(int c = 1; c<= this->num_vertices -1; c++)
        {
            bellman_ford_pass(dists);
        }
